#!/bin/bash
# Post-train a cross-embodiment checkpoint onto bridge alone (EE-cartesian actions,
# regardless of which family BASE was originally trained with — see
# clap.eval.experiments's comment on _POST_TRAIN_BRIDGE for why).
...
# Usage: BASE=oxe_lam_clap bash examples/posttrain/posttrain_bridge.sh [--override key=val (e.g., training.max_train_steps=1) ...]
set -euo pipefail
source "$(dirname "$0")/../_common.sh"

BASE=${BASE:-oxe_ee}
TAG="post_train_${BASE}_bridge"
RESET=true
